---
permalink: /
title: "About Me"
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

<div class="about-flex">
  <div class="about-main">
    <p>I am an M.S. student in Electrical and Electronic Engineering at Yonsei University, advised by Prof. Jong-Moon Chung in the Communications &amp; Networking Laboratory (CNL). My research focuses on <strong>learning-based management and optimization of dynamic wireless networks</strong> — with equal interest in algorithm design and validation on real systems. <strong>I am applying to Ph.D. programs in ECE/CS for Fall 2027.</strong></p>

    <div class="tag-chips">
      <span class="chip">Machine Learning for Wireless Networks</span>
      <span class="chip">O-RAN & Network Slicing</span>
      <span class="chip">Time-Sensitive Scheduling</span>
      <span class="chip">Network Resource Management</span>
      <span class="chip">Multi-access Edge Computing</span>
      <span class="chip">6G Non-Terrestrial Networks(NTN)</span>
    </div>

    <h2>Research Focus</h2>
    <ul>
      <li><strong>Learning-based network control</strong> — Machine learning for traffic and mobility management in dynamic wireless networks</li>
      <li><strong>O-RAN and network slicing</strong> — Reinforcement learning for radio resource allocation across network slices, with mathematical analysis providing per-slice SLA guarantees</li>
      <li><strong>Multi-access edge computing (MEC)</strong> — Distributing AI inference across device, edge, and network for real-time, low-latency services</li>
      <li><strong>AI on real wireless systems</strong> — Validating learning-based methods on real systems (e.g., private 5G MEC, UAV platforms), bridging simulation and deployment</li>
      <li><strong>Non-terrestrial networks</strong> — Network management for dynamic NTNs such as LEO satellite and UAV systems</li>
    </ul>
  </div>

  <aside class="news-card">
    <h2>📢 News</h2>

    <div class="news-item">
      <span class="news-date">JUL 2026</span>
      <p>Paper published in <strong>IEEE Wireless Communications Letters</strong>: <em>GT-PAR: Graph Transformer-Aided Traffic Prediction and Adaptive Routing for Dynamic LEO Satellite Networks</em> (first author).</p>
    </div>

    <div class="news-item">
      <span class="news-date">JUL 2026</span>
      <p><strong>Outstanding Oral Presentation Award</strong>, 8th Industry–Academia Project Showcase of the MOTIE/KIAT Future Vehicle Core Technology Workforce Consortium.</p>
    </div>

    <div class="news-item">
      <span class="news-date">JUN 2026</span>
      <p><strong>Outstanding Paper Award</strong>, 2026 Spring Conference of KIECS.</p>
    </div>

    <div class="news-item">
      <span class="news-date">MAY 2026</span>
      <p>Paper published in <strong>IEEE Transactions on Mobile Computing</strong>: <em>Intelligent Handover Scheme for Improved 6G NTN LEO Satellite Network Performance</em> (co-author).</p>
    </div>
  </aside>
</div>
